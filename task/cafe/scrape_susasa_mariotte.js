...
function parseArgs(argv) {
  ...
  if (!args.out) {
    const fileName = `susasa_mariotte_${koreaDateKey()}.json`;
...
function printHelp() {
  console.log(`
Usage:
  node task/cafe/scrape_susasa_mariotte.js --start-page 1 --end-page 200

Options:
  --start-page <number>     시작 페이지 (default: 1)
  --end-page <number>       종료 페이지 (default: 200)
  --per-page <number>       페이지당 글 수 (default: 15)
  --out <path>              출력 JSON 경로 (default: docs/exam/susasa_mariotte_YYYYMMDD.json)
...
async function run() {
  ...
  console.log(
    [
      `스사사 메리어트 게시판 수집 시작: page ${args.startPage}-${args.endPage}`,
      `output: ${args.out}`,

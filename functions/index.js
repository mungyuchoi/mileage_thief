...
// ── 세계지도(milecatch) 스탬프 소비: 서버 권위 트랜잭션 ──────────
...
exports.worldUnlockCountry = onCall({region: OAUTH_REGION}, async (request) => {
  ...
  return db.runTransaction(async (tx) => {
    ...
    if (unlockSnap.exists) {
      return {already: true, passportStamps: balance};
    }
    if (balance < cost) {
    ...
    return {already: false, passportStamps: next};
  });
});
...
exports.worldUnlockSlots = onCall({region: OAUTH_REGION}, async (request) => {
  ...
  const db = admin.firestore();
  const walletRef = db.doc(`users/${uid}/exploreWallet/main`);
  const slotRef = db.doc(`users/${uid}/exploreSlots/${countryId}`);
  return db.runTransaction(async (tx) => {
    ...
    const nextCap = cap + WORLD_SLOT_STEP;
    const nextBalance = balance - cost;
    tx.set(walletRef, {passportStamps: nextBalance}, {merge: true});
    tx.set(slotRef, {
      cap: nextCap,
      updatedAt: admin.firestore.FieldValue.serverTimestamp(),
    }, {merge: true});

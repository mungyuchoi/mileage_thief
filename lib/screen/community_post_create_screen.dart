...
class _CommunityPostCreateScreenState extends State<CommunityPostCreateScreen> {
  ...
  void initState() {
    ...
    if (widget.isEditMode) {
      ...
      // 기존 이미지 개수 파싱
      if (widget.editContentHtml != null &&
          widget.editContentHtml!.isNotEmpty) {
        ...
        tempImagePaths = List.generate(
            existingImageCount, (index) => 'EXISTING_IMAGE_$index');
      }
    } else {
      // 새 게시글 작성 모드
      selectedBoardId = widget.initialBoardId;
  ...
  Future<void> _checkTempDataAndShowDialog() async {
    try {
      ...
      if ((tempTitle != null && tempTitle.isNotEmpty) ||
          (tempContent != null &&
              tempContent.isNotEmpty &&
              tempContent.trim() != '<p></p>') ||
  ...
  Future<void> _restoreHtmlContent() async {
    try {
      ...
      if (tempContent != null &&
          ...
          tempContent.trim() != '<p></p>') {
        print(
            '임시 저장된 HTML 내용 복원: ${tempContent.length > 100 ? tempContent.substring(0, 100) + "..." : tempContent}');
  ...
  Future<void> _saveTempData() async {
    try {
      ...
      if (title.isNotEmpty ||
          (content.isNotEmpty && content.trim() != '<p></p>')) {
    ...
    try {
      ...
      if (image != null) {
        ...
        final String imageHtml =
            '<img src="file://${compressedFile.path}" style="max-width: 100%; border-radius: 8px;" /><br/>';
  ...
  Future<String> _uploadImageAndGetUrl(
      String imagePath, String postId, String dateString) async {
    try {
      ...
      final String fileName =
          '${postId}_${const Uuid().v4()}.${imagePath.split('.').last}';
  ...
  Future<String> _processImagesInHtml(
      String htmlContent, String postId, String dateString) async {
    String processedHtml = htmlContent;

    print('HTML 처리 시작, 원본 크기:  [33m${htmlContent.length} [0m 바이트');

    // file:// 형태의 로컬 이미지 경로 처리
    final RegExp fileImgRegex =
        RegExp(r'<img[^>]*src="file://([^"]*)"[^>]*>', caseSensitive: false);
    ...
    for (final match in matches) {
      ...
      try {
        ...
        final String downloadUrl =
            await _uploadImageAndGetUrl(imagePath, postId, dateString);
        print('로컬 이미지 업로드 완료: $downloadUrl');

        // HTML에서 로컬 경로를 다운로드 URL로 교체
        final String newImgTag =
            '<img src="$downloadUrl" style="max-width: 100%; border-radius: 8px;" />';
  ...
  Future<void> _handleSubmit() async {
    ...
    try {
      ...
      print(
          'HTML 내용 미리보기 (첫 1000자): ${contentHtml.length > 1000 ? contentHtml.substring(0, 1000) : contentHtml}');
      ...
      final base64Count =
          RegExp(r'data:image/[^;]+;base64,').allMatches(contentHtml).length;
      ...
      print(
          '원본 contentHtml (첫 500자): ${contentHtml.length > 500 ? contentHtml.substring(0, 500) : contentHtml}');

      final processedContentHtml =
          await _processImagesInHtml(contentHtml, postId, dateString);

      print('처리된 contentHtml 크기: ${processedContentHtml.length} 바이트');
      print(
          '처리된 contentHtml (첫 500자): ${processedContentHtml.length > 500 ? processedContentHtml.substring(0, 500) : processedContentHtml}');
      ...
      print(
          'contentHtml: ${postData['contentHtml']?.toString().length ?? 0} 바이트');

      // contentHtml이 여전히 클 경우 추가 처리
      if (processedContentHtml.length > 900000) {
        // 900KB 이상일 경우
              ...
              children: [
                ...
                GestureDetector(
                  onTap: () async {
                    ...
                    if (result is Map<String, dynamic>) {
                      ...
                      print(
                          '선택된 게시판: ID=$selectedBoardId, Name=$selectedBoardName');
                    }
                  },
                  child: Container(
                ...
                HtmlEditor(
                  ...
                  callbacks: Callbacks(
                    ...
                    onInit: () async {
                      ...
                      // 편집 모드일 때 기존 내용 설정
                      if (widget.isEditMode &&
                          widget.editContentHtml != null &&
                          widget.editContentHtml!.isNotEmpty) {
                        print(
                            '편집 모드 - 기존 내용 설정: ${widget.editContentHtml!.length > 100 ? widget.editContentHtml!.substring(0, 100) + "..." : widget.editContentHtml!}');
                ...
                Container(
                  ...
                  child: ElevatedButton.icon(
                    onPressed: tempImagePaths.length >= maxImageCount
                        ? null
                        : _addImageToEditor,
                    icon: Icon(

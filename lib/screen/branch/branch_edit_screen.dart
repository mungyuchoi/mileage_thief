...
class _BranchEditScreenState extends State<BranchEditScreen> {
    ...
    for (final entry in openingHours.entries) {
      ...
      if (key == 'monFri') {
        ...
        days = ['mon', 'tue', 'wed', 'thu', 'fri', 'sat', 'sun'];
          ...
          dayOrder.contains(key.substring(0, 3)) &&
          dayOrder.contains(key.substring(3, 6))) {
        // 조합된 키 (예: tueWed)
  ...
  Future<void> _saveToFirestore({
    ...
    try {
      ...
      final existingOwnerUid = widget.branchData['createdByUid'] as String?;

      // 지점 정보 업데이트. 기존 소유자는 편집자와 분리해서 보존한다.
      await FirebaseFirestore.instance
        ...
        'createdByUid':
            existingOwnerUid?.isNotEmpty == true ? existingOwnerUid : uid,

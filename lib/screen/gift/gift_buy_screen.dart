...
class _GiftBuyScreenState extends State<GiftBuyScreen> {
  ...
  bool _hasTemplateReference(List<Map<String, dynamic>> list, String key, String? id) {
    if (id == null || id.isEmpty) return false;
    return list.any((item) => (item[key] as String?) == id);
  }

  String? _templateNameById(List<Map<String, dynamic>> list, String key, String? id) {
  ...
  CollectionReference<Map<String, dynamic>> _templateCollection(String uid) {
    return FirebaseFirestore.instance.collection('users').doc(uid).collection('gift_templates');
  }
  ...
  String _formatTemplateSummary(Map<String, dynamic> template) {
    ...
    final mode = _asString(payload['priceInputMode']) == 'discount' ? 'discount' : 'buyUnit';
    final giftName = _templateNameById(_giftcards, 'giftcardId', giftcardId) ?? '상품권 미확인';
    final cardName = _templateNameById(_cards, 'cardId', cardId) ?? '카드 미확인';
    final whereName = _templateNameById(_whereToBuys, 'whereToBuyId', whereToBuyId);
    final priceText = mode == 'discount' ? '할인율 ${discount.toStringAsFixed(2)}%' : '매입가 ${buyUnit.toString()}원';
    final memo = _asString(payload['memo'])?.trim();
    final locationText = whereName == null ? '' : ' / $whereName';
    final memoText = (memo == null || memo.isEmpty) ? '' : ' / ${memo.length > 12 ? '${memo.substring(0, 12)}...' : memo}';
  ...
  Map<String, dynamic> _buildTemplatePayloadFromCurrentForm() {
    final faceValue = _asInt(_faceValueController.text.replaceAll(RegExp(r'[^0-9]'), ''));
    final buyUnit = _asInt(_buyUnitController.text.replaceAll(RegExp(r'[^0-9]'), ''));
  ...
  Future<void> _createTemplateFromCurrentForm() async {
    ...
    final name = await _askTemplateName(title: '템플릿 저장', initialName: defaultName);
  ...
  Future<void> _overwriteTemplateWithCurrentForm(Map<String, dynamic> template) async {
    final uid = FirebaseAuth.instance.currentUser?.uid;
  ...
  Future<void> _renameTemplate(Map<String, dynamic> template) async {
    ...
    final nextName = await _askTemplateName(title: '템플릿 이름 수정', initialName: currentName);
  ...
  Future<void> _deleteTemplate(Map<String, dynamic> template) async {
    ...
    final confirm = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text('이 템플릿을 삭제하시겠습니까?', style: TextStyle(color: Colors.black)),
        actions: [
          TextButton(
          ...
          TextButton(
            ...
            child: const Text('삭제', style: TextStyle(color: Colors.black, fontWeight: FontWeight.bold)),
          ),
        ],
      ),
    );
  ...
  Future<void> _applyTemplate(Map<String, dynamic> template) async {
    ...
    final String mode = _asString(payload['priceInputMode']) == 'discount' ? 'discount' : 'buyUnit';
    ...
    final String? giftcardValue = (giftcardId != null && _hasTemplateReference(_giftcards, 'giftcardId', giftcardId)) ? giftcardId : null;
    final String? cardValue = (cardId != null && _hasTemplateReference(_cards, 'cardId', cardId)) ? cardId : null;
    final String? whereValue = (whereToBuyId != null && _hasTemplateReference(_whereToBuys, 'whereToBuyId', whereToBuyId)) ? whereToBuyId : null;
    if (giftcardId != null && giftcardId.isNotEmpty && giftcardValue == null) cleared.add('상품권');
    if (cardId != null && cardId.isNotEmpty && cardValue == null) cleared.add('카드');
    if (whereToBuyId != null && whereToBuyId.isNotEmpty && whereValue == null) cleared.add('구매처');
    ...
    if (cleared.isNotEmpty) {
      Fluttertoast.showToast(msg: '삭제된 참조 정리: ${cleared.join(', ')} 선택값이 비워졌습니다.');
    }
              ...
              child: Column(
                ...
                children: [
                  ...
                  Row(
                    children: [
                      ...
                      ElevatedButton(
                        ...
                        child: const Text('현재값 저장', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w700)),
                      ),
                    ],
                  ),
                  ...
                  if (_templatesLoading)
                    ...
                    const Expanded(
                      child: Center(
                        child: Text('저장된 템플릿이 없습니다.', style: TextStyle(color: Colors.black54)),
                      ),
                    )
                  else
                    Expanded(
                      child: ListView.separated(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          final String name = _asString(template['name']) ?? '이름 없음';
                          return ListTile(
                            dense: true,
                            title: Text(name, style: const TextStyle(color: Colors.black, fontWeight: FontWeight.w700)),
                            subtitle: Text(
                              ...
                              children: [
                                ...
                                PopupMenuButton<String>(
                                  ...
                                  onSelected: (value) async {
                                    Navigator.pop(sheetContext);
                                    if (value == 'overwrite') {
                                      await _overwriteTemplateWithCurrentForm(template);
      ...
      appBar: AppBar(
        ...
        actions: [
          TextButton.icon(
            ...
            label: const Text('템플릿', style: TextStyle(color: Colors.black, fontWeight: FontWeight.w700)),
                ...
                child: SingleChildScrollView(
                  ...
                  children: [
                    ...
                    if (pinnedTemplates.isNotEmpty) ...[
                      const Text('고정 템플릿', style: TextStyle(fontWeight: FontWeight.w700, color: Colors.black)),
                      ...
                      Wrap(
                        ...
                        children: pinnedTemplates.map((template) {
                          ...
                          return ActionChip(
                            ...
                            shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(999), side: const BorderSide(color: Color(0xFFCFD3DD))),
                            label: Text(name),

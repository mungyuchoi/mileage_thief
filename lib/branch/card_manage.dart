  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
          ...
          IconButton(
            ...
            onPressed: () async {
              await Navigator.push(context,
                  MaterialPageRoute(builder: (_) => const CardStepPage()));
            },
          ),
        ],
      ),
      body: uid == null
          ? const Center(
              child:
                  Text('로그인이 필요합니다.', style: TextStyle(color: Colors.black54)))
              ...
              builder: (context, snap) {
                if (snap.connectionState == ConnectionState.waiting) {
                  return const Center(
                      child: CircularProgressIndicator(
                          valueColor:
                              AlwaysStoppedAnimation(Color(0xFF74512D))));
                }
                ...
                return ListView.separated(
                  ...
                  itemBuilder: (context, i) {
                    ...
                    final targetSpend =
                        (data['targetSpendKRW'] as num?)?.toInt() ?? 0;
                          ...
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                ...
                                const SizedBox(height: 4),
                                Text('신용: $credit  |  체크: $check',
                                    style: const TextStyle(
                                        color: Colors.black87, fontSize: 12)),
                                if (targetSpend > 0) ...[
                                  const SizedBox(height: 4),
                                  Text('월 목표 실적: $targetSpend원',
                                      style: const TextStyle(
                                          color: Colors.black87, fontSize: 12)),
                                ],
                                if (memo.isNotEmpty) ...[

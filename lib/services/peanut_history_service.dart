...
class PeanutHistoryService {
  ...
  static Future<void> addHistory({
    ...
    try {
      ...
      // 추가 데이터가 있으면 병합
      final finalData =
          additionalData != null ? {...baseData, ...additionalData} : baseData;

      await historyRef.set(finalData);
    } catch (e) {
      debugPrint('땅콩 히스토리 추가 오류: $e');
    }
  }
  ...
  static String getDisplayTitle(String type) {
    switch (type) {
      ...
      case 'admin_gift':
        return '운영자 선물';
      case 'mock_exam_submit_reward':
        return '마일고사 응시 보상';
      case 'mock_exam_retry_purchase':
        return '마일고사 재도전';
      default:
        return '땅콩 획득';
    }
  ...
  static String getDisplaySubtitle(Map<String, dynamic> data) {
    ...
    switch (type) {
      ...
      case 'mock_exam_submit_reward':
        return data['reason'] as String? ?? '첫 응시 제출 보상';
      case 'mock_exam_retry_purchase':
        return data['reason'] as String? ?? '재도전권 구매';
      default:
        return '';
    }
  }

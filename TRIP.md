...
| 필드명 | 타입 | 설명 | 예시 |
| :--- | :--- | :--- | :--- |
| `deal_id` | String | 문서 고유 ID (노선+날짜+편명 조합) | `ICN_LAX_20250125_YP103_YP102` |
...
### A. `deal_id` 생성 규칙 (중요)
한 날짜에 여러 비행편이 있는 경우를 해결하기 위해 ID에 편명을 조합합니다.
- **규칙**: `{출발공항}_{도착공항}_{출발날짜}_{가는편명}_{오는편명}`
- **예시**: `ICN_LAX_20250125_YP0103_YP0102`
- **효과**: 같은 날짜/노선이라도 시간대(편명)가 다르면 별개의 특가 정보로 저장되어 UI에 각각 노출될 수 있습니다.

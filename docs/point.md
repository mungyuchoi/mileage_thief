...
6. `pointAwardCandidates`는 `calendarYears`를 기준으로 다시 계산한 파생 인덱스다. 모든 원천 날짜를 후보 문서로 중복 저장하지 않는다.
...
v1에서는 `nights` 1~7박 후보를 미리 만든다. 8박 이상은 후보 문서를 만들지 않고, 호텔 상세의 `calendarYears`를 기준으로 계산하는 방식으로 확장한다.
...
8. 1~7박 기준으로 `pointAwardCandidates`를 생성 또는 갱신한다.
9. 이번 실행에서 확인되지 않은 기존 후보는 즉시 삭제하지 않고 `status: inactive` 또는 `stale: true`로 표시한다.
